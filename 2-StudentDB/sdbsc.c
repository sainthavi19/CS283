...
{
    student_t temp;
    lseek(fd, 0, SEEK_SET);  // Start from the beginning of the file
    while (read(fd, &temp, sizeof(student_t)) > 0)
    {
        if (temp.id == id)
        {
            *s = temp;  // Copy the found student data into s
            return NO_ERROR;
        }
    }
...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    while (read(fd, &temp, sizeof(student_t)) > 0)
    {
        if (temp.id == id)
        {
            printf(M_ERR_DB_ADD_DUP);  // Student already exists
            return ERR_DB_OP;
        }
    }

    // Move the file descriptor to the end for appending new student
    lseek(fd, 0, SEEK_END);

    student_t new_student = {id, fname, lname, gpa};
    if (write(fd, &new_student, sizeof(student_t)) == -1)
    {
        printf(M_ERR_DB_WRITE);
        return ERR_DB_FILE;
    }
    printf(M_STD_ADDED);  // Success message
    return NO_ERROR;
}
...
int del_student(int fd, int id)
{
    ...
    while (read(fd, &temp, sizeof(student_t)) > 0)
    {
        if (temp.id == id)
        {
            student_t empty_student = {0};  // Empty student record
            lseek(fd, -sizeof(student_t), SEEK_CUR);  // Move the file descriptor back to the student's position
            if (write(fd, &empty_student, sizeof(student_t)) == -1)
            {
                printf(M_ERR_DB_WRITE);
                return ERR_DB_FILE;
            }
            printf(M_STD_DEL_MSG);  // Success message
            return NO_ERROR;
        }
    }
    printf(M_STD_NOT_FND_MSG, id);  // Student not found
    return ERR_DB_OP;
}
...
int count_db_records(int fd)
{
    ...
    while (read(fd, &temp, sizeof(student_t)) > 0)
    {
        if (memcmp(&temp, EMPTY_STUDENT_RECORD, sizeof(student_t)) != 0)  // If it's not an empty record
        {
            count++;
        }
    }
    ...
    {
        printf(M_DB_RECORD_CNT, count);  // Print the count of records
    }
    return count;
}
...
int print_db(int fd)
{
    ...
    while (read(fd, &temp, sizeof(student_t)) > 0)
    {
        if (memcmp(&temp, EMPTY_STUDENT_RECORD, sizeof(student_t)) != 0)  // Skip empty records
        {
            if (!printed_header)
            {
            ...
            printf(STUDENT_PRINT_FMT_STRING, temp.id, temp.fname, temp.lname, temp.gpa / 100.0);  // Convert GPA to float
        }
    }
    return NO_ERROR;
}
...
int compress_db(int fd)
{
    ...
    while (read(fd, &temp, sizeof(student_t)) > 0)
    {
        ...
        {
            if (write(temp_fd, &temp, sizeof(student_t)) == -1)
            {
                printf(M_ERR_DB_WRITE);
                close(temp_fd);
                return ERR_DB_FILE;
